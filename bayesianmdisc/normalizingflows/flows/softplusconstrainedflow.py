...
class SoftplusConstrainedFlow(torch.nn.Module):
    def __init__(
    ...
    def inverse(self, x: Tensor) -> tuple[Tensor, Tensor]:
        ...
        def u_func() -> Tensor:
            constr_u = torch.log(torch.exp(constr_x) - one)
            return torch.concat((constr_u, unconstr_x), dim=1)[:, self._inv_permutation]

        def log_det_func() -> Tensor:
            d_constr_u_dx = one / (one - torch.exp(-constr_x))
...
def create_softplus_constrained_flow(
    total_num_outputs: int, indices_constrained_outputs: list[int], device: Device
) -> SoftplusConstrainedFlow:
    return SoftplusConstrainedFlow(
        total_num_outputs, indices_constrained_outputs, device
    )
