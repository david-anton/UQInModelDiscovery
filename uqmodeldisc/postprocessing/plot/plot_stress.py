...
from uqmodeldisc.customtypes import Device, NPArray, Tensor
from uqmodeldisc.data import interpolate_heteroscedastic_noise
from uqmodeldisc.data.anisotropicheartdataset import (
    assemble_flattened_deformation_gradients,
    generate_principal_stretches,
)
from uqmodeldisc.datasettings import create_four_terms_anisotropic_model_parameters
from uqmodeldisc.errors import PlotterError
from uqmodeldisc.gps.base import GPMultivariateNormal
from uqmodeldisc.gps.gp import GP
from uqmodeldisc.gps.multioutputgp import IndependentMultiOutputGP
from uqmodeldisc.io import ProjectDirectory
from uqmodeldisc.models import IsotropicModel, ModelProtocol, OrthotropicCANN
from uqmodeldisc.models.base_mechanics import assemble_stretches_from_factors
...
from uqmodeldisc.statistics.metrics import (
...
from uqmodeldisc.utility import from_numpy_to_torch, from_torch_to_numpy
...
def plot_model_stresses_treloar(
    ...
    def plot_one_input_output_set(
        ...
        text = "\n".join(
            (
                r"$EC_{95\%}=$" + r"${0}\%$".format(round(coverage, 2)),
...
def plot_model_stresses_anisotropic(
    ...
    figure.tight_layout(
        w_pad=plotter_config.pad_subplots_width, h_pad=plotter_config.pad_subplots_hight
    )
    axes[5, 1].axis("off")
    axes[5, 2].axis("off")
    subfigure_counter = 0
    ...
    def plot_one_data_set(
        ...
        def plot_one_stress(
            ...
            axis.plot(
                ...
                label=plotter_config.model_mean_label,
            )
            axis.fill_between(
                model_inputs_axis,
            ...
            axis.set_xticks(x_ticks)
            if subfigure_counter in data_config.subfigure_indices_with_xaxis:
