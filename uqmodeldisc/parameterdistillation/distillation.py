...
from uqmodeldisc.customtypes import (
    Device,
    Module,
    Tensor,
    TorchLRScheduler,
...
from uqmodeldisc.gps import GaussianProcess
from uqmodeldisc.gps.base import GPMultivariateNormal
from uqmodeldisc.io import ProjectDirectory
from uqmodeldisc.io.loaderssavers import PytorchModelLoader, PytorchModelSaver
from uqmodeldisc.models import ModelProtocol, OutputSelectorProtocol
from uqmodeldisc.networks.ffnn import FFNN
from uqmodeldisc.normalizingflows import (
    NormalizingFlowDistribution,
    NormalizingFlowProtocol,
)
from uqmodeldisc.parameterdistillation.parameterdistributions import (
...
from uqmodeldisc.testcases import TestCases
from uqmodeldisc.utility import flatten_outputs
...
class LipschitzFunction(nn.Module):
    ...
    def __call__(self, funcs: Tensor) -> Tensor:
        return self._network(funcs)

    def _init_network(self) -> Module:
        hidden_layer_size = int(math.floor(self._rel_layer_width * self._func_size))
        layer_sizes = [self._func_size]
...
def _infer_gp_distribution(gp: GaussianProcess, inputs: Tensor) -> GPMultivariateNormal:
    # return gp.infer_predictive_distribution(inputs, noise_stddevs)
    return gp(inputs)


def _create_distribution_optimizer(
    distribution: ParameterDistribution, learning_rate: float
) -> TorchOptimizer:
...
def _create_lipschitz_func_optimizer(
    lipschitz_func: Module, learning_rate: float
) -> TorchOptimizer:
...
def distill_parameter_distribution_from_gp(
    ...
    def wasserstein_loss(gp_funcs: Tensor, model_funcs: Tensor) -> Tensor:
        lipschitz_outputs_gp = lipschitz_func(gp_funcs)
        lipschitz_outputs_model = lipschitz_func(model_funcs)
        return torch.mean(lipschitz_outputs_gp - lipschitz_outputs_model)

    def lipschitz_loss(gp_funcs: Tensor, model_funcs: Tensor) -> Tensor:
        def gradient_penalty(gp_funcs: Tensor, model_funcs: Tensor) -> Tensor:
            one = torch.tensor(1.0, device=device)
            single_lipschitz_func = lambda func: lipschitz_func(func)[0]
            ...
            vmap_grad_penalty = lambda func_values: (
                torch.square(
                    l2_norm(grad(single_lipschitz_func, argnums=0)(func_values)) - one
                )
            )
            return torch.mean(vmap(vmap_grad_penalty)(combined_funcs))

        return -wasserstein_loss(
            gp_funcs, model_funcs
        ) + lipschitz_coefficient * gradient_penalty(gp_funcs, model_funcs)
    ...
    lipschitz_func = LipschitzFunction(
        func_size=num_flattened_outputs,
    ...
    _freeze_gp(gp)
    gp_distribution = _infer_gp_distribution(gp, inputs)

    optimizer_distribution = _create_distribution_optimizer(
        distribution, initial_lr_distribution
    )
    optimizer_lipschitz = _create_lipschitz_func_optimizer(
        lipschitz_func, initial_lr_lipschitz_func
    )
    lr_scheduler_distribution = _create_lr_scheduler(
        optimizer_distribution, lr_decay_rate_distribution
    )
    lr_scheduler_lipschitz_func = _create_lr_scheduler(
        optimizer_lipschitz, lr_decay_rate_lipschitz_func
    )
    wasserstein_loss_hist = []

    for iter_wasserstein in range(1, num_iters_wasserstein + 1):
        for _ in range(num_iters_lipschitz):
            gp_funcs = draw_gp_func_values()
            model_funcs = draw_model_func_values()

            def lipschitz_loss_closure() -> float:
                optimizer_lipschitz.zero_grad(set_to_none=True)
                loss_lipschitz = lipschitz_loss(gp_funcs, model_funcs)
        ...
        def wasserstein_loss_closure() -> float:
            ...
            loss_wasserstein = wasserstein_loss(gp_funcs, model_funcs)
