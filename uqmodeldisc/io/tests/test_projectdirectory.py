...
from uqmodeldisc.errors import DirectoryNotFoundError, FileNotFoundError
from uqmodeldisc.io.projectdirectory import ProjectDirectory
from uqmodeldisc.settings import Settings


class FakeSetting(Settings):
    def __init__(self) -> None:
        self.PROJECT_DIR = Path() / "uqmodeldisc" / "io" / "tests"
...
def test_check_if_file_exists_for_existing_output_file(sut: ProjectDirectory) -> None:
    file_name = "existing_output_file.txt"
    subdir_name = "output_subdirectory"
    subdir_path = settings.PROJECT_DIR / settings.OUTPUT_SUBDIR / subdir_name
    ...
    actual = sut.check_if_file_exists(
        file_name=file_name, search_in_output_dir=True, subdir_name=subdir_name
    )
...
def test_check_if_file_exists_for_not_existing_output_file(
    sut: ProjectDirectory,
) -> None:
    file_name = "not_existing_output_file.txt"
    subdir_name = "output_subdirectory"
    subdir_path = settings.PROJECT_DIR / settings.OUTPUT_SUBDIR / subdir_name
    Path.mkdir(subdir_path, parents=True)
...
def test_check_if_file_exists_for_existing_input_file(sut: ProjectDirectory) -> None:
    file_name = "existing_input_file.txt"
    subdir_name = "output_subdirectory"
    subdir_path = settings.PROJECT_DIR / settings.INPUT_SUBDIR / subdir_name
    file_path = subdir_path / file_name
    Path.mkdir(subdir_path, parents=True)
    ...
    actual = sut.check_if_file_exists(
        file_name=file_name, search_in_output_dir=False, subdir_name=subdir_name
    )
...
def test_check_if_file_exists_for_not_existing_intput_file(
    sut: ProjectDirectory,
) -> None:
    file_name = "not_existing_intput_file.txt"
    subdir_name = "output_subdirectory"
    subdir_path = settings.PROJECT_DIR / settings.INPUT_SUBDIR / subdir_name
    Path.mkdir(subdir_path, parents=True)
